...
@LoadModule
object TitleManager {
    private val config get() = Awesome.config.debug.title
    ...
    private fun command(args: Array<String>) {
        if (args.size != 3) {
            ChatUtils.warning("Wrong usage! /awtesttitle <duration> <fadeIn> <fadeOut>")
            return
        }
        val title = config.title.replace("&", "§")
        val subtitle = config.subtitle.replace("&", "§")
    ...
    private fun renderTitle(partialTicks: Float) {
        ...
        if (alpha.coerceIn(0, 255) > 8) {
            ...
            fontRenderer.drawString(
                title,
                (-fontRenderer.getStringWidth(title) / 2).toFloat(),
                -10.0f,
                0xFFFFFF or color,
                true,
            )
            ...
            fontRenderer.drawString(
                subTitle,
                (-fontRenderer.getStringWidth(subTitle) / 2).toFloat(),
                5.0f,
                0xFFFFFF or color,
                true,
            )
